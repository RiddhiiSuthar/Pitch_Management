...
def pitchUpdate(request, id):  
    ...
    form = PitchForm(initial={'name': pitch.name, 
...
def update_pitch_data_with_weather(request, pitch_id):
    ...
    if weather_data:
        ...
        if pitch.weather == "Rain":
            ...
            pitch.next_maintenance_date = pitch.last_maintenance_date + timedelta(days=settings.REGULAR_MAINTENANCE_DAY)
        pitch.save()
...
def pitchlist(request):
    ...
    pitch = Pitch.objects.filter(next_maintenance_date__gte=datetime.now() ).order_by('next_maintenance_date')
